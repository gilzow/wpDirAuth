...
define('WPDIRAUTH_LDAP_RETURN_KEYS',serialize(array(
    'last_name'         =>  'sn',
    ...
    function wpDirAuth_check_oldpassword($objUser,$strPassword)
    {
        if(!wpdirauth_already_changed_password($objUser)){
            ...
            if(wp_check_password($strPassword,$objUser->data->user_pass,$objUser->ID)){
                ...
                wp_set_password(wp_generate_password(24),$objUser->ID);
            }
...
________EOS;
    ...
    function wpDirAuth_hidePassFields()
    {
        ...
        if (!$editUserIsDirUser) {
            ...
            if (get_site_option('dirAuthTOS')) {
                // TOS option is ON
                if (($TOSDate = get_user_meta($user_id, 'wpDirAuthTOS', true)) === '') {
    ...
    function wpDirAuth_retrieveUserDetails($rscConnection,$strBaseDn,$strFilterQuery,$rscResult=false){
        ...
        if(!$rscResult){
            ...
            if ($intCount < 1) {
                ...
                $aryLdapKeys = apply_filters('wpdirauth_ldap_user_keys',unserialize(WPDIRAUTH_LDAP_RETURN_KEYS));

                $strEmail       = isset($aryUserDetails[0][$aryLdapKeys['email']][0]) ? $aryUserDetails[0][$aryLdapKeys['email']][0] : '';

                $strLastName    = isset($aryUserDetails[0][$aryLdapKeys['last_name']][0]) ? $aryUserDetails[0][$aryLdapKeys['last_name']][0] : '';

                $strFirstName   = isset($aryUserDetails[0][$aryLdapKeys['first_name']][0]) ? $aryUserDetails[0][$aryLdapKeys['first_name']][0] : '';
            ...
            if(wp_verify_nonce($_POST['_wpnonce_add-da-user'],'add-da-user')){
                ...
                if(isset($_POST['ssoid']) && $_POST['ssoid'] == ''){
                    ...
                    if(is_wp_error($aryUserData)){
                        ...
                        $strSiteProtocol = (1 == get_site_option('dirAuthRequireSsl')) ? 'https' : 'http';
                        ...
                        if(isset($arySitesAndRoles) && count($arySitesAndRoles)!=0){
                            foreach($arySitesAndRoles as $arySiteData){
                                $arySitesAddedTo[] = array(
                                    ...
                                    'siteurl' =>get_site_url($arySiteData['blog_id'],'',$strSiteProtocol),
                            ...
                            $arySitesAddedTo[] = array(
                                ...
                                'siteurl'   =>site_url('',$strSiteProtocol),
                            );
                        }
    ...
    {
        $aryReturnKeys = array_values(unserialize(WPDIRAUTH_LDAP_RETURN_KEYS));
